...
def __get_thumbnail__(obj, image=None):
    if not image:
        if hasattr(obj, 'get_main_image'):
    ...
    if image:
        ...
        try:
            path = get_thumbnailer(image)['thumb'].url
            html = format_html('<img src="{}">'.format(path), )
        except InvalidImageFormatError:
            html = format_html(
                '<b style="color:red">{}</b>'.format(
